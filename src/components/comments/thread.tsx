...
export function CommentThread({
  ...
  return (
    <div className="rounded-[14px] border border-navy/8 bg-white p-4">
      <button
        type="button"
        className="flex w-full items-center justify-between text-start text-sm font-semibold"
        onClick={() => setOpen((v) => !v)}
      >
        <span>
        ...
        <span className="text-[11px] font-normal text-navy/40">{open ? "Hide" : "Show"}</span>
      </button>
      {open ? (
        <>
